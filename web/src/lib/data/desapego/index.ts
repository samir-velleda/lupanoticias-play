/**
 * Repositório Desapegoo — mock em memória (dev) com API estável para Aurora depois.
 */
...
export const desapegoRepo = {
  ...
  async getById(id: string): Promise<DesapegoAnuncio | null> {
    return clone(_anuncios.find((a) => a.id === id)) ?? null;
  },

  async getBySlug(slug: string): Promise<DesapegoAnuncio | null> {
    return clone(_anuncios.find((a) => a.slug === slug)) ?? null;
  },

  async getVendedorBySlug(slug: string): Promise<DesapegoVendedor | null> {
    return clone(_vendedores.find((v) => v.slug === slug)) ?? null;
  },

  async listVendedores(): Promise<DesapegoVendedor[]> {
    return clone(_vendedores);
  },

  async criar(input: CriarDesapegoAnuncioInput): Promise<DesapegoAnuncio> {
